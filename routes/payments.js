...
router.delete('/:id/images/:imageId', requireAuth, async (req, res) => {
  const user = req.session.user;
  try {
    // Verify the payment exists and the user has permission
...
router.delete('/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Destroy all images from Cloudinary (child table)
    ...
    // Also clean up legacy screenshot column if still populated
    if (rows[0].screenshot_public_id) {
      await cloudinary.uploader.destroy(rows[0].screenshot_public_id).catch(() => {});
    }

    // Cascade in DB handles deleting payment_images rows automatically
